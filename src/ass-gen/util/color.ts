 ...
 */
const isDarkColor = ({ r, g, b }: RGB) => r * 0.299 + g * 0.587 + b * 0.114 < 0x30;
...
export const hexColorToRGB = (hex: string) => {
  ...
  const [r, g, b] = hex.length === 3 ? hex.split("").map((c) => c + c) : hex.match(/../g) || [];
...
export const getDecoratingColor = (color: RGB) => (isDarkColor(color) ? WHITE : BLACK);

export const isWhite = (color: RGB) => color.r === 255 && color.g === 255 && color.b === 255;
