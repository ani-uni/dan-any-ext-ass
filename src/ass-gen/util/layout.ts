...
import { DanmakuList2UDanmakus, UDanmakus2DanmakuLists } from "./danconvert.ts";
import { arrayOfLength } from "./lang.ts";

// 计算一个矩形移进屏幕的时间（头进屏幕到尾巴进屏幕）
const computeScrollInTime = (rectWidth: number, screenWidth: number, scrollTime: number) => {
  const speed = (screenWidth + rectWidth) / scrollTime;
  return rectWidth / speed;
};

// 计算一个矩形在屏幕上的时间（头进屏幕到头离开屏幕）
const computeScrollOverTime = (rectWidth: number, screenWidth: number, scrollTime: number) => {
  const speed = (screenWidth + rectWidth) / scrollTime;
...
export const measureTextWidthConstructor = (canvasContext: CanvasCtx) => {
  ...
  if (supportTextMeasure) {
    return (fontName: string, fontSize: number, bold: boolean, text: string) => {
...
const initializeLayout = (config: SubtitleStyle, canvasCtx: CanvasCtx) => {
  ...
  return (danmaku: Danmaku) => {
    ...
    const rectWidth =
      measureTextWidthConstructor(canvasCtx)(fontName, danmakuFontSize, bold, danmaku.content) +
      paddingLeft +
      paddingRight;
    ...
    if (danmaku.type === DanmakuType.SCROLL) {
      ...
      const gridNumber = resolveAvailableFixGrid(targetGrids as FixGrid[], danmaku.time);
      if (gridNumber < 0) {
        // console.warn(`[Warn] Collision ${danmaku.time}: ${danmaku.content}`)
        return null;
      }
...
export const layoutDanmaku = (
  ...
  const list = [...UDanmakus2DanmakuLists(inputList)].toSorted((x, y) => x.time - y.time);
  const layout = initializeLayout(config, canvasCtx);

  return DanmakuList2UDanmakus(list.map(layout).filter((danmaku) => !!danmaku));
};
